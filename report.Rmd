...
```
Notamos, também, várias linhas com NAs em campos importantes, como o número de porta ou o tipo de local, pelo que optamos por eliminar essas linhas, bem como as que poderiam ter valores inválidos.

```{r remove, eval=FALSE}
crimes <- crimes[complete.cases(crimes[,1:7]),]
crimes <- crimes[!grepl("[0-9]+", crimes$OffenType),]
```
